...
def dated_jsonl(directory: Path) -> Path:
    return directory / f"takeover_{dt.datetime.now().strftime('%Y%m%d')}.jsonl"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="ChatBot V3.3-17 canary expansion smoke")
    ...
    tests = [
        {
            "name": "allowed_v3_3_17_general_expanded",
            "user": "v3_3_17_takeover",
            "conversation_id": f"v3-3-17-takeover-{ts}-general-expanded",
            "message": "解释一下 OSPF 邻居状态 Full 和 2-Way 的区别，不要生成命令。",
            "expect_takeover": True,
            "expect_action": "general_chat",
        },
        {
            "name": "allowed_v3_3_17_advice_expanded",
            "user": "v3_3_17_takeover",
            "conversation_id": f"v3-3-17-takeover-{ts}-advice-expanded",
            "message": "是否建议在重启 standby 网络设备前先隔离流量？只给运维建议，不要生成命令。",
            "expect_takeover": True,
            "expect_action": "advice_analysis",
        },
        {
            "name": "compat_v3_3_16_general",
            "user": "v3_3_16_takeover",
            "conversation_id": f"v3-3-16-takeover-{ts}-compat-general",
        ...
        },
        {
            ...
            "user": "normal_user",
            "conversation_id": f"v3-3-17-takeover-{ts}-blocked-user",
            "message": "解释一下 OSPF 邻居状态 Full 和 2-Way 的区别，不要生成命令。",
        ...
        {
            "name": "blocked_cmdb_query",
            "user": "v3_3_17_takeover",
            "conversation_id": f"v3-3-17-takeover-{ts}-blocked-cmdb",
            "message": "查一下 V3-3-17-TAKEOVER-NONEXIST-DEVICE-001 的管理 IP 和设备类型",
            "expect_takeover": False,
        },
    ]

    audit_file = dated_jsonl(audit_dir)
    ...
    for index, test in enumerate(tests, start=1):
        ...
        if item["v3_audit_error"]:
            raise SystemExit(f"ERROR: audit write failed and was exposed in response for {test['name']}: {item}")
        if expected_takeover:
            ...
            if item["v3_audit_error"]:
                raise SystemExit(f"ERROR: unexpected audit error for {test['name']}: {item}")
    ...
    if after_audit_lines < before_audit_lines + len(tests):
        raise SystemExit(
            f"ERROR: audit log did not grow enough: before={before_audit_lines}, after={after_audit_lines}, tests={len(tests)}"
        )

    audit_tail = []
    if audit_file.exists():
        lines = audit_file.read_text(encoding="utf-8", errors="replace").splitlines()
        for line in lines[-max(20, len(tests)):]:
            try:
    ...
    if taken_count < 3:
        raise SystemExit(f"ERROR: audit taken_count too low: {taken_count}")
    if blocked_count < 3:
        raise SystemExit(f"ERROR: audit blocked_count too low: {blocked_count}")

    report = {
        "version": "v3.3.17",
        "purpose": "low-risk canary expansion with audit observability",
    ...
    (report_dir / "v3_3_17_takeover_expansion_report.json").write_text(
        json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True),
        encoding="utf-8",
    )

    print("v3_3_17_allowed_general_expanded=OK")
    print("v3_3_17_allowed_advice_expanded=OK")
    print("v3_3_17_compat_v3_3_16=OK")
    print("v3_3_17_blocked_user=OK")
    print("v3_3_17_blocked_prefix=OK")
    print("v3_3_17_blocked_cmdb_query=OK")
    print("v3_3_17_audit_observability=OK")
    print("v3_3_17_takeover_expansion_summary=OK")
    return 0
